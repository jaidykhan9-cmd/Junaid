...
export const CustomerPortal: React.FC<CustomerPortalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md overflow-y-auto">
      <div className="relative w-full max-w-2xl bg-[#0e1017] border border-[#d4af37]/35 rounded-xl overflow-hidden shadow-2xl my-6">
        
        {/* Header */}
        <div className="bg-[#141724] border-b border-[#d4af37]/20 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <h2 className="font-display font-bold text-base text-[#f8fafc]">
                JD Client Portal
              </h2>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-1 rounded text-[#94a3b8] hover:text-[#f8fafc] hover:bg-white/5 cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 max-h-[75vh] overflow-y-auto space-y-6">
          ...
          {/* User Sign-In Banner */}
          {user ? (
            <div className="p-4 rounded-lg bg-[#121520] border border-[#d4af37]/30 flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                {user.photoURL ? (
                  <img src={user.photoURL} alt={user.displayName || ''} className="w-10 h-10 rounded-full border border-[#d4af37]" />
                ) : (
                  <div className="w-10 h-10 rounded-full bg-[#1c2234] border border-[#d4af37] flex items-center justify-center text-[#d4af37] font-bold">
                    {user.displayName?.[0] || 'C'}
                  </div>
                )}
                <div>
                  <div className="text-xs font-bold text-[#f8fafc]">{user.displayName || 'Valued Client'}</div>
              ...
              <div className="flex items-center gap-2">
                {isAdmin && (
                  <button
                    ...
                    className="px-3 py-1.5 bg-[#d4af37] text-[#08090d] rounded text-xs font-bold cursor-pointer hover:bg-[#fae8b4] transition-colors"
                  >
                    Open Admin Console
                  </button>
                )}
                <button
                  ...
                  className="px-3 py-1.5 bg-white/5 hover:bg-white/10 text-xs text-[#94a3b8] rounded border border-white/10 cursor-pointer"
                >
                  Sign Out
                </button>
              </div>
            ...
            <div className="p-5 rounded-lg bg-[#121520] border border-[#d4af37]/25 text-center">
          ...
          <div className="p-4 rounded-lg bg-[#121520] border border-white/10">
            <h4 className="text-xs font-bold text-[#fae8b4] mb-1">
            ...
            <form onSubmit={handleSearchByPhone} className="flex gap-2">
              <input
                ...
                className="flex-1 bg-[#0a0c12] border border-white/10 rounded-lg px-3 py-2 text-xs text-[#f8fafc] focus:outline-none focus:border-[#d4af37]"
              />
              <button
                type="submit"
                className="px-4 py-2 bg-[#1c2234] hover:bg-[#252c42] border border-[#d4af37]/40 text-[#fae8b4] rounded text-xs font-bold cursor-pointer transition-colors"
              >
                Search
              </button>
            </form>
          ...
          <div>
            ...
            {(() => {
              ...
              if (listToDisplay.length === 0) {
                return (
                  <div className="p-6 text-center text-xs text-[#94a3b8] border border-white/5 rounded-lg">
              ...
              return (
                <div className="space-y-3">
                  {listToDisplay.map((b) => (
                    <div
                      key={b.id}
                      className="p-4 rounded-lg bg-[#121520] border border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3"
                    >
                      <div>
                        <div className="flex items-center gap-2">
                        ...
                        <div className="font-semibold text-xs text-[#f8fafc] mt-1">{b.serviceTitle}</div>
                      ...
                      <div className="flex items-center gap-2 self-end sm:self-center">
                        {b.paymentStatus === 'UNPAID' && (
                          <button
                            ...
                            className="px-3 py-1.5 bg-[#d4af37] text-[#08090d] text-xs font-bold rounded flex items-center gap-1.5 cursor-pointer hover:bg-[#fae8b4] transition-colors"
                          >

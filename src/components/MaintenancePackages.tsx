...
export const MaintenancePackages: React.FC<MaintenancePackagesProps> = ({ onSelectPackage }) => {
  return (
    <section id="packages" className="py-24 bg-[#090b10] relative border-t border-[#d4af37]/20">
            ...
            return (
              <div
                ...
                className={`relative rounded-xl p-8 flex flex-col justify-between transition-all duration-300 ${
                  isFeatured
                    ? 'bg-gradient-to-b from-[#1c2234] via-[#141824] to-[#0f111a] border-2 border-[#d4af37] shadow-[0_16px_50px_rgba(212,175,55,0.22)] md:-translate-y-2'
                    : 'luxury-card'
                }`}
              >
                {isFeatured && (
                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-gradient-to-r from-[#fae8b4] via-[#d4af37] to-[#aa820a] text-[#08090d] text-[11px] font-bold uppercase tracking-widest px-3.5 py-1 rounded-full shadow-md">
                ...
                <div>
                  ...
                  <h3 className="font-display font-bold text-2xl text-[#f8fafc] mt-1">
                    {pkg.name}
                  </h3>
                  ...
                  <div className="mt-6 pb-6 border-b border-white/10">
                    <div className="flex items-baseline gap-2">
                      <span className="font-display font-extrabold text-3xl text-[#f8fafc] tabular-nums">
                        {pkg.pricePKR}
                      </span>
                      <span className="text-xs text-[#94a3b8]">/ {pkg.period}</span>
                    </div>
                  ...
                  <ul className="mt-6 space-y-3">
                    {pkg.inclusions.map((item, iIdx) => (
                      <li key={iIdx} className="flex items-start gap-3 text-xs text-[#cbd5e1] leading-normal">
                ...
                <div className="mt-8 pt-6 border-t border-white/10">
                  <button
                    ...
                    className={`w-full py-3 px-4 rounded text-xs font-semibold uppercase tracking-wider transition-all flex items-center justify-center gap-2 cursor-pointer ${
                      isFeatured
                        ? 'bg-[#d4af37] hover:bg-[#fae8b4] text-[#08090d] shadow-lg font-bold'
                        : 'bg-white/5 hover:bg-[#d4af37] hover:text-[#08090d] text-[#f8fafc] border border-white/10 font-medium'
                    }`}
                  >

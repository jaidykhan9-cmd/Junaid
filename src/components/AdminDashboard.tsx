...
import { GmailHub } from './GmailHub.tsx';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ isOpen, onClose }) => {
  const [activeTab, setActiveTab] = useState<
    'overview' | 'bookings' | 'payments' | 'reviews' | 'careers' | 'customers' | 'audit' | 'gmail'
  >('overview');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/85 backdrop-blur-md overflow-hidden">
      <div className="relative w-full max-w-7xl h-[92vh] bg-[#0e1119] border border-[#d4af37]/40 rounded-xl overflow-hidden shadow-2xl flex flex-col">
        
        {/* Top Header */}
        <div className="bg-[#141824] border-b border-[#d4af37]/20 px-6 py-4 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded bg-[#181d2c] border border-[#d4af37] flex items-center justify-center font-serif-lux font-bold text-lg text-[#d4af37]">
              JD
            </div>
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] bg-[#d4af37]/15 border border-[#d4af37]/40 text-[#fae8b4] px-2 py-0.5 rounded font-mono">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="p-2 rounded text-[#94a3b8] hover:text-[#f8fafc] hover:bg-white/5 transition-colors cursor-pointer"
              title="Refresh Data"
            >
            ...
            <button
              onClick={onClose}
              className="p-2 rounded text-[#94a3b8] hover:text-white hover:bg-white/5 cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        {/* Action Toast Banner */}
        {actionMessage && (
          <div className="bg-[#d4af37] text-[#08090d] text-xs font-bold px-6 py-2 flex items-center justify-between shadow">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {/* Sidebar Navigation */}
          <aside className="w-60 bg-[#0d1017] border-r border-[#d4af37]/15 p-4 flex flex-col justify-between shrink-0 hidden md:flex">
            <nav className="space-y-1 text-xs">
              <button
                onClick={() => setActiveTab('overview')}
                className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded font-medium transition-colors cursor-pointer ${
                  activeTab === 'overview'
                    ? 'bg-[#141824] text-[#fae8b4] border border-[#d4af37]/30'
              ...
              >
                <div className="flex items-center gap-2.5">
                  <Mail className="w-4 h-4 text-[#fae8b4]" />
                  <span>Gmail Workspace Hub</span>
                </div>
                <span className="text-[10px] bg-[#d4af37]/20 border border-[#d4af37]/40 text-[#fae8b4] px-1.5 py-0.5 rounded font-mono font-bold">
                  API
                </span>
              </button>
            ...
            <div className="p-3 rounded bg-[#0e1119] border border-white/5 text-[11px] text-[#94a3b8]">
          ...
          {/* Main Content Area */}
          <main className="flex-1 p-6 overflow-y-auto bg-[#08090d]">
            ...
            {activeTab === 'overview' && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="p-4 rounded-xl bg-[#10131d] border border-[#d4af37]/20">
                    <div className="text-xs text-[#94a3b8]">Total Recorded Bookings</div>
                    <div className="font-display font-extrabold text-2xl text-[#f8fafc] mt-1">
                      {stats?.totalBookings ?? bookings.length}
                    </div>
                    <div className="text-[11px] text-[#d4af37] mt-0.5">PostgreSQL Managed</div>
                  </div>

                  <div className="p-4 rounded-xl bg-[#10131d] border border-red-800/40">
                    <div className="text-xs text-[#94a3b8]">Pending Dispatch</div>
                  ...
                  <div className="p-4 rounded-xl bg-[#10131d] border border-amber-800/40">
                    <div className="text-xs text-[#94a3b8]">Pending JazzCash Verification</div>
                  ...
                  <div className="p-4 rounded-xl bg-[#10131d] border border-emerald-800/40">
                    <div className="text-xs text-[#94a3b8]">Verified Revenue (PKR)</div>
                ...
                <div className="p-5 rounded-xl bg-[#10131d] border border-[#d4af37]/25 space-y-4">
                  ...
                  {bookings.slice(0, 5).map((b) => (
                    <div
                      key={b.id}
                      className="p-3 rounded-lg bg-[#0a0c12] border border-white/5 flex flex-wrap items-center justify-between gap-3 text-xs"
                    >
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-mono font-bold text-[#fae8b4]">{b.bookingId}</span>
                          <span className="font-semibold text-[#f8fafc]">{b.customerName}</span>
                          <span className="text-[#94a3b8]">({b.phone})</span>
                        </div>
                        <div className="text-[#38bdf8] mt-0.5">{b.serviceTitle}</div>
                        <div className="text-[11px] text-[#64748b]">{b.address}</div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="px-2.5 py-1 bg-[#161b28] text-[#fae8b4] border border-[#d4af37]/40 rounded hover:bg-[#1f2538]"
                        >
                          Details
                        </button>
                      </div>
              ...
              <div className="space-y-4">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="bg-[#0a0c12] text-[#94a3b8] border-b border-[#d4af37]/20">
                    ...
                    <tbody className="divide-y divide-white/5">
                      {bookings.map((b) => (
                        <tr key={b.id} className="hover:bg-[#10131d] transition-colors">
                          <td className="p-3 font-mono font-bold text-[#fae8b4]">{b.bookingId}</td>
                          <td className="p-3">
                            <div className="font-semibold text-[#f8fafc]">{b.customerName}</div>
                            <div className="text-[11px] text-[#94a3b8]">{b.phone}</div>
                          </td>
                          <td className="p-3 text-[#f8fafc] max-w-xs truncate">{b.serviceTitle}</td>
                          <td className="p-3 text-[#94a3b8]">{b.preferredDate}</td>
                          <td className="p-3">
                            <select
                              value={b.bookingStatus}
                              onChange={(e) => handleUpdateBookingStatus(b.bookingId, e.target.value)}
                              className="bg-[#0a0c12] border border-white/10 rounded px-2 py-1 text-xs text-[#f8fafc]"
                            >
                          ...
                          <td className="p-3 text-right space-x-1">
                            <button
                              onClick={() => setSelectedBooking(b)}
                              className="px-2 py-1 bg-white/5 hover:bg-white/10 rounded text-[11px] text-[#f8fafc]"
                            >
                              Inspect
                            </button>
                          </td>
            ...
            {activeTab === 'payments' && (
              <div className="space-y-4">
                ...
                {payments.length === 0 ? (
                  <div className="p-8 text-center text-xs text-[#94a3b8] border border-white/5 rounded-lg">
                  ...
                  <div className="space-y-3">
                    {payments.map((p) => (
                      <div
                        key={p.id}
                        className="p-4 rounded-lg bg-[#10131d] border border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-4 text-xs"
                      >
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-mono font-bold text-sm text-[#fae8b4]">
                              Rs. {p.amount.toLocaleString()}
                            </span>
                            <span className="text-[#94a3b8]">via {p.paymentMethod}</span>
                            <span
                          ...
                          <div className="mt-1 space-y-0.5 text-[11px] text-[#94a3b8]">
                            <div>Booking ID: <strong className="text-[#f8fafc] font-mono">{p.bookingId}</strong></div>
                            <div>Transaction TID: <strong className="text-[#38bdf8] font-mono">{p.transactionRef}</strong></div>
                            <div>Sender Mobile: <strong className="text-[#f8fafc]">{p.senderNumber}</strong></div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {reviews.map((r) => (
                    <div
                      key={r.id}
                      className="p-4 rounded-lg bg-[#10131d] border border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs"
                    >
                      <div>
                        <div className="flex items-center gap-2">
                          <strong className="text-[#f8fafc]">{r.customerName}</strong>
                        ...
                        <p className="mt-1 text-[#94a3b8] italic">"{r.comment}"</p>
                        {r.bookingId && (
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {applications.map((app) => (
                    <div
                      key={app.id}
                      className="p-4 rounded-lg bg-[#10131d] border border-white/10 text-xs space-y-2"
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <strong className="text-[#f8fafc] text-sm">{app.fullName}</strong>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-[11px] text-[#94a3b8]">
                        <div>Phone: <strong className="text-[#f8fafc]">{app.phone}</strong></div>
                        <div>Email: <strong className="text-[#f8fafc]">{app.email}</strong></div>
                      ...
                      {app.message && (
                        <p className="text-[#94a3b8] bg-[#0a0c12] p-2 rounded italic">
                          "{app.message}"
                        </p>
                      )}
                      ...
                      <div className="pt-2 flex items-center gap-2">
                        ...
                        <a
                          href={`tel:${app.phone}`}
                          className="px-3 py-1 bg-white/5 border border-white/10 text-[#f8fafc] rounded text-[11px]"
                        >
                          Call
                        </a>
                      </div>
                    ...
                    <tbody className="divide-y divide-white/5">
                      {customers.map((c) => (
                        <tr key={c.id}>
                          ...
                          <td className="p-3 text-[#f8fafc] font-semibold">{c.fullName}</td>
                          <td className="p-3 text-[#94a3b8]">{c.email}</td>
                          <td className="p-3 text-[#94a3b8]">{c.phone || 'N/A'}</td>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {auditLogs.map((log) => (
                    <div
                      key={log.id}
                      className="p-3 rounded bg-[#0a0c12] border border-white/5 flex items-center justify-between text-xs"
                    >
                      <div>
                        <span className="font-mono font-bold text-[#d4af37] mr-2">[{log.action}]</span>
                        <span className="text-[#f8fafc]">{log.entityType} ({log.entityId}):</span>
                        <span className="text-[#94a3b8] ml-1">{log.details}</span>
                      </div>
            ...
            {/* GMAIL WORKSPACE HUB TAB */}
            {activeTab === 'gmail' && (
              <GmailHub bookings={bookings} onBookingUpdated={fetchAllData} />
            )}
        ...
        {selectedBooking && (
          <div className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <div className="relative w-full max-w-xl bg-[#10131d] border border-[#d4af37] rounded-xl p-6 shadow-2xl text-xs space-y-4">
              <div className="flex items-center justify-between border-b border-white/10 pb-3">
                <div>
                  <div className="font-mono font-bold text-sm text-[#fae8b4]">
                ...
                <button onClick={() => setSelectedBooking(null)} className="p-1 text-[#94a3b8] hover:text-white">
                  <X className="w-5 h-5" />
                </button>
              </div>

              <div className="space-y-2 text-[#94a3b8]">
                <div>Customer: <strong className="text-[#f8fafc]">{selectedBooking.customerName}</strong> ({selectedBooking.phone}, {selectedBooking.email})</div>
                <div>Service: <strong className="text-[#f8fafc]">{selectedBooking.serviceTitle}</strong></div>
                <div>Schedule: <strong className="text-[#f8fafc]">{selectedBooking.preferredDate} ({selectedBooking.preferredTime})</strong></div>
                <div>Address: <strong className="text-[#f8fafc]">{selectedBooking.address}</strong></div>
                ...
                <div className="p-3 bg-[#0a0c12] rounded border border-white/5">
                  <span className="font-bold text-[#d4af37] block mb-1">Problem Description:</span>
                  <p className="whitespace-pre-wrap text-[#f8fafc]">{selectedBooking.problemDescription}</p>
                </div>
              </div>
              ...
              <div className="pt-2 flex flex-wrap gap-2">
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-[#d4af37] text-[#08090d] font-bold rounded flex items-center gap-1.5 hover:bg-[#fae8b4] cursor-pointer"
                >
                  <Mail className="w-3.5 h-3.5" />
                  <span>Open in Gmail Hub</span>
                </button>
                <button
                  onClick={() => handleResendEmail(selectedBooking.bookingId)}
                  className="px-3 py-1.5 bg-[#161b28] text-[#fae8b4] border border-[#d4af37]/40 rounded"
                >
                  Resend Alert
                </button>
                <button
                  onClick={() => setSelectedBooking(null)}
                  className="px-3 py-1.5 bg-white/5 text-[#94a3b8] rounded"
                >
                  Close
                </button>
              </div>
